...
class MM_RRT_ABMIL(nn.Module):
    ...
    def __init__(self, 
        ...
        if fusion_type == 'two_stage_region' and num_modalities > 1:
            ...
            elif self.stage2_type == 'he_residual_cross_v3':
            ...
            elif self.stage2_type == 'he_residual_cross_v4':
                from models.he_residual_cross_crmsa_v4 import HEResidualCrossCRMSAv4
                self.cross_region_mod = HEResidualCrossCRMSAv4(
                    dim=mlp_dim,
                    num_heads=_get(stage2_cfg, 'crmsa_heads', 8),
                    region_num=_get(stage2_cfg, 'region_num', 4),
                    crmsa_k=_get(stage2_cfg, 'crmsa_k', 3),
            ...
            elif self.stage2_type == 'he_residual_cross_v5':
                from models.he_residual_cross_crmsa_v5 import HEResidualCrossCRMSAv5
                self.cross_region_mod = HEResidualCrossCRMSAv5(
                    dim=mlp_dim,
                    num_heads=_get(stage2_cfg, 'crmsa_heads', 8),
                    region_num=_get(stage2_cfg, 'region_num', 4),
                    crmsa_k=_get(stage2_cfg, 'crmsa_k', 3),
    ...
    def forward(self, x, return_features=False, return_modality_attns=False):
        ...
        if self.fusion_type == 'two_stage_region' and len(x_emb_list) > 1:
            ...
            if self.stage2_type == 'staining_msa':
                ...
                z_final = self.cross_region_mod(z_he, z_ihc)
                fusion_stats = {'two_stage_region': True, 'stage2': 'he_residual_cross_v3'}
            elif self.stage2_type == 'he_residual_cross_v4':
                z_final = self.cross_region_mod(z_he, z_ihc)
                fusion_stats = {'two_stage_region': True, 'stage2': 'he_residual_cross_v4'}
            elif self.stage2_type == 'he_residual_cross_v5':
                z_final, aux = self.cross_region_mod(z_he, z_ihc)
                fusion_stats = {
                    'two_stage_region': True,
                    'stage2': 'he_residual_cross_v5',
                }
                if aux is not None:

"""
R-MSA / CR-MSA / TransLayer — exact match with official RRT-MIL-master.

Source: /home/Public/lillan/RRT-MIL-master/modules/rmsa.py
        /home/Public/lillan/RRT-MIL-master/modules/rrt.py (TransLayer)
"""
...
# ---------------------------------------------------------------------------
# Region partition / reverse  (identical to official)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# InnerAttention — exact match with official
# ---------------------------------------------------------------------------

class InnerAttention(nn.Module):
    """Inner (window) attention with EPEG.  Exact match with official rmsa.py."""

    def __init__(self, dim, head_dim=None, num_heads=8, qkv_bias=True, qk_scale=None,
                 attn_drop=0., proj_drop=0., epeg=True, epeg_k=15,
                 epeg_2d=False, epeg_bias=True, epeg_type='attn', **kwargs):
        super().__init__()
        _ = kwargs  # consume unused kwargs (crmsa_k, fixed_beta, etc.)
        self.dim = dim
        ...
        if epeg:
            ...
            if epeg_2d:
                if epeg_type == 'attn':
                    self.pe = nn.Conv2d(num_heads, num_heads, epeg_k,
                ...
                if epeg_type == 'attn':
                    ...
                    self.pe = nn.Conv2d(head_dim * num_heads, head_dim * num_heads, (epeg_k, 1),
    ...
    def forward(self, x):
        ...
        # EPEG on value (before attention output)
        ...
        # EPEG on output (after attention)
...
# ---------------------------------------------------------------------------
# RegionAttention  (R-MSA) — exact match with official RegionAttntion
# ---------------------------------------------------------------------------

class RegionAttention(nn.Module):
    """Region-based Multi-head Self-Attention (R-MSA).  Exact match with official."""
    ...
    def __init__(self, dim, head_dim=None, num_heads=8, region_size=0, qkv_bias=True,
                 ...
                 min_region_num=0, min_region_ratio=0., region_attn='native', **kwargs):
        super().__init__()
...
# ---------------------------------------------------------------------------
# CrossRegionAttention  (CR-MSA) — exact match with official CrossRegionAttntion
# ---------------------------------------------------------------------------

class CrossRegionAttention(nn.Module):
    """Cross-Region Multi-head Self-Attention (CR-MSA).  Exact match with official."""

    def __init__(self, dim, head_dim=None, num_heads=8, region_size=0, qkv_bias=True,
                 qk_scale=None, drop=0., attn_drop=0., region_num=8, epeg=False,
                 min_region_num=0, min_region_ratio=0., crmsa_k=3, crmsa_mlp=False,
                 region_attn='native', **kwargs):
        super().__init__()
...
# ---------------------------------------------------------------------------
# TransLayer — exact match with official rrt.py TransLayer
# ---------------------------------------------------------------------------

class TransLayer(nn.Module):
    """Single transformer layer for RRT.  Exact match with official rrt.py."""

    def __init__(self, norm_layer=nn.LayerNorm, dim=512, head=8, drop_out=0.1,
                 drop_path=0., ffn=False, ffn_act='gelu', mlp_ratio=4.,
                 trans_dim=64, attn='rmsa', n_region=8, epeg=False,
                 region_size=0, min_region_num=0, min_region_ratio=0,
                 qkv_bias=True, crmsa_k=3, epeg_k=15, **kwargs):
        super().__init__()
        ...
        if attn == 'ntrans':
            ...
            raise NotImplementedError(f"Unknown attn type: {attn}")
        ...
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim,
                       act_layer=act_layer, drop=drop_out) if ffn else nn.Identity()

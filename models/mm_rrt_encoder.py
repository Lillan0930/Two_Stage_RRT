...
from models.rmsa import (RegionAttention, CrossRegionAttention, Mlp,
                          region_partition, region_reverse, TransLayer)
...
# ---------------------------------------------------------------------------
# Position Embedding stubs (pos='none' by default → Identity)
# ---------------------------------------------------------------------------

class PEG(nn.Module):
    """Position Embedding Generator — stub (only used when pos='peg')."""
    def __init__(self, dim=512, k=7, bias=True, conv_1d=False):
        super().__init__()
        if conv_1d:
            self.proj = nn.Conv2d(dim, dim, (k, 1), 1, (k // 2, 0), groups=dim, bias=bias)
        else:
            self.proj = nn.Conv2d(dim, dim, k, 1, k // 2, groups=dim, bias=bias)
...
class PPEG(nn.Module):
    """Pyramid PEG — stub (only used when pos='ppeg')."""
    def __init__(self, dim=512, k=7, conv_1d=False, bias=True):
        super().__init__()
        kw = (k, 1) if conv_1d else k
        pad = (k // 2, 0) if conv_1d else k // 2
        self.proj = nn.Conv2d(dim, dim, kw, 1, pad, groups=dim, bias=bias)
        self.proj1 = nn.Conv2d(dim, dim, (5, 1) if conv_1d else 5, 1,
                               (2, 0) if conv_1d else 2, groups=dim, bias=bias)
        self.proj2 = nn.Conv2d(dim, dim, (3, 1) if conv_1d else 3, 1,
                               (1, 0) if conv_1d else 1, groups=dim, bias=bias)
...
class SINCOS(nn.Module):
    """Sinusoidal 2D position embedding — stub (only used when pos='sincos')."""
    def __init__(self, embed_dim=512):
        super().__init__()
        self.embed_dim = embed_dim
        self.pos_embed = self._get_2d_sincos(embed_dim, 8)
    ...
    @classmethod
    def _get_2d_sincos(cls, embed_dim, grid_size):
        grid_h = np.arange(grid_size, dtype=np.float32)
        grid_w = np.arange(grid_size, dtype=np.float32)
        grid = np.meshgrid(grid_w, grid_h)
        grid = np.stack(grid, axis=0).reshape([2, 1, grid_size, grid_size])
        emb_h = cls._get_1d_sincos(embed_dim // 2, grid[0])
        emb_w = cls._get_1d_sincos(embed_dim // 2, grid[1])
        return np.concatenate([emb_h, emb_w], axis=1)

    def forward(self, x):
        B, H, W, C = x.shape
        pos_embed = torch.from_numpy(self.pos_embed).float().to(x.device)
        return x + pos_embed.unsqueeze(1).unsqueeze(1).repeat(1, H, W, 1)


# ---------------------------------------------------------------------------
# Official RRTEncoder — exact match with official rrt.py RRTEncoder
# ---------------------------------------------------------------------------

class RRTEncoder(nn.Module):
    """
    Single-modality R²T encoder — EXACT match with official RRT-MIL-master rrt.py.

    Flow:
      [B, N, C] → R-MSA layers (TransLayer) → CR-MSA (TransLayer) →
      all_shortcut residual → LayerNorm → [B, N, C]

    Usage (two-stage first stage):
      he_encoder = RRTEncoder(mlp_dim=512, epeg_k=9, crmsa_k=3, ...)
      z_he = he_encoder(x_he)   # [B, N, 512]
    """
    ...
    def __init__(self, mlp_dim=512, pos_pos=0, pos='none', peg_k=7,
                 ...
                 mlp_ratio=4., trans_dim=64, epeg=True, epeg_k=15,
                 ...
                 crmsa_mlp=False, crmsa_heads=8, need_init=False, **kwargs):
        ...
        # R-MSA layers (n_layers-1 layers of TransLayer with attn='rmsa')
        self.layers = []
        for i in range(n_layers - 1):
            self.layers += [TransLayer(
                dim=mlp_dim, head=n_heads, drop_out=drop_out,
                drop_path=drop_path, ffn=ffn, ffn_act=ffn_act,
                mlp_ratio=mlp_ratio, trans_dim=trans_dim,
                attn=attn, n_region=region_num, epeg=epeg,
        ...
        # CR-MSA (wrapped in TransLayer with attn='crmsa')
        self.cr_msa = TransLayer(
            dim=mlp_dim, head=crmsa_heads, drop_out=drop_out,
            drop_path=drop_path, ffn=ffn, ffn_act=ffn_act,
            mlp_ratio=mlp_ratio, trans_dim=trans_dim,
            attn='crmsa', qkv_bias=qkv_bias, crmsa_k=crmsa_k,
            crmsa_mlp=crmsa_mlp, **kwargs) if cr_msa else nn.Identity()

        # Position embedding (default: none → Identity)
        if pos == 'ppeg':
    ...
    def forward(self, x):
        ...
        # CR-MSA (with TransLayer wrapper: Norm → CRSA → residual + DropPath)
        x = self.cr_msa(x)

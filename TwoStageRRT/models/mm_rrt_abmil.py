...
class MM_RRT_ABMIL(nn.Module):
    ...
    def __init__(self, 
        ...
        self.rrt_encoder_2 = None  # separate encoder for IHC (two_stage_separate)
        if fusion_type == 'two_stage_region' and num_modalities > 1:
            ...
            if kwargs.get('two_stage_separate_encoder', False):
                self.rrt_encoder_2 = MM_RRTEncoder(
                    mlp_dim=mlp_dim, num_modalities=1,
                    fusion_type='self_attention', fusion_stage=fusion_stage,
                    region_num=region_num, n_layers=n_layers, n_heads=n_heads,
                    drop_path=drop_path, drop_out=trans_dropout, epeg=epeg,
                    epeg_k=epeg_k, crmsa_k=crmsa_k, cr_msa=cr_msa,
                    all_shortcut=all_shortcut, aggregate_modalities=True, need_init=True,
                )
    ...
    def forward(self, x, return_features=False, return_modality_attns=False):
        ...
        # ══════════════════════════════════════════════════════════
        # ═══ Two-stage Region R²T (shared or separate encoder) ═══
        if self.fusion_type == 'two_stage_region' and len(x_emb_list) > 1:
            # Stage 1: HE encoder (shared)
            z_he = self.rrt_encoder([x_emb_list[0]])
            z_he = z_he[0] if isinstance(z_he, tuple) else z_he
            # IHC encoder (shared or separate)
            ihc_encoder = self.rrt_encoder_2 if self.rrt_encoder_2 is not None else self.rrt_encoder
            z_ihc = ihc_encoder([x_emb_list[1]])
            z_ihc = z_ihc[0] if isinstance(z_ihc, tuple) else z_ihc

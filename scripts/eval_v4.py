#!/usr/bin/env python3
"""HE Residual Cross v4 — post-training evaluation.

Sections (run with --section, or all by default):
  repro        re-run pure inference on 3 v4 checkpoints, compare saved AUC
  causal       matched / random-mismatch(5) / within-class-mismatch(5) / disable
  diagnostics  12 per-slide diagnostics: entropy_norm, score_std, value_diversity,
               selective_ratio, mu_norm, r_slide, r_region, PLUS q_raw_cos,
               q_centered_cos, k_raw_cos, k_centered_cos, attn_weight_std, plus
               rho (= ||0.1·Δ||/||z_he||).
  summary      main result table (v4 vs HE-only/v1/v2/v3 + causal) → _eval/summary.json

Outputs land in results/stage2_he_residual_cross_v4/_eval/.
"""
...
V3_ROOT = PROJECT / "results" / "stage2_he_residual_cross_v3"
...
@torch.inference_mode()
def compute_v4_diag(model, he, pr):
...
def section_repro(device):
    out = {}
    for s in SEEDS:
        seed_dir = RESULTS_ROOT / "he_residual_cross_v4" / f"seed{s}"
...
def section_summary(device):
    ...
    v4_auc = {str(s): repro[str(s)]["recomputed_auc"] for s in SEEDS}
    ...
    for rs in REPLACEMENT_SEEDS:
        ...
        wc_mm[str(rs)] = {str(s): causal[str(s)]["within_class_mismatch"][str(rs)]["auc_all"]
                          for s in SEEDS}
    # mean over replacement seeds (per model seed)
    rand_mm_mean = {str(s): float(np.mean([rand_mm[str(rs)][str(s)]
                                           for rs in REPLACEMENT_SEEDS]))
                    for s in SEEDS}
    wc_mm_mean = {str(s): float(np.mean([wc_mm[str(rs)][str(s)]
                                         for rs in REPLACEMENT_SEEDS]))
                  for s in SEEDS}

    # §10 headline deltas
    delta_he = _delta(matched, he_only_auc)
    delta_cross = _delta(matched, disable)
    delta_random = _delta(matched, rand_mm_mean)
    delta_within = _delta(matched, wc_mm_mean)

    summary = {
        "task": "stage2_he_residual_cross_v4_eval",
        "evaluation_protocol": "Train / Val-as-Test",
        "v4_per_seed_auc": v4_auc,
        "v4_mean_auc": _mean_std(v4_auc)[0],
        "v4_std_auc": _mean_std(v4_auc)[1],
        "reused_baselines": {
            "he_only": he_only_auc, "v1": v1_auc, "v2": v2_auc, "v3": v3_auc,
        },
        "paired_delta_auc": {
            "v4_minus_he": _delta(v4_auc, he_only_auc),
            "v4_minus_v1": _delta(v4_auc, v1_auc),
            "v4_minus_v2": _delta(v4_auc, v2_auc),
            "v4_minus_v3": _delta(v4_auc, v3_auc),
        },
        "causal": {
            "matched_auc": matched,
            "disable_cross_auc": disable,
            "matched_minus_disable_mean": delta_cross["mean"],
        ...
        "headline_deltas": {
            "delta_he": delta_he,          # matched − HE-only
            "delta_cross": delta_cross,    # matched − disable_cross
            "delta_random": delta_random,  # matched − random_mismatch(mean over repl)
            "delta_within": delta_within,  # matched − within_class_mismatch(mean over repl)
        },
        "diagnostics": diag,
    }

#!/usr/bin/env python3
"""
PR+RRT+ABMIL Optuna worker —— 从共享 SQLite study 拉 trial，在本 GPU 上训练。

协议（对齐 Base_mil）：
  train = c16_train_labels.csv (270)
  val   = c16_test_labels.csv  (129)   # test-as-val，early stopping 直接选 test 上最优 epoch
  objective = best_val_auc；early_stopping monitor val_auc / max / patience 10；num_epochs=80 / cosine。

搜索空间（6 参数，网格已搜过的结构/损失参数全部固定）：
  n_heads ∈ {2,4,8} | abmil_hidden_dim ∈ {128,256,384} | drop_path ∈ [0,0.3]
  lr ∈ log[3e-5,3e-4] | wd ∈ log[1e-6,1e-4] | label_smoothing ∈ [0,0.2]

用法（由 run_pr_rrt_optuna.py 自动启动，或手动）：
  CUDA_VISIBLE_DEVICES=6 python scripts/_run_pr_rrt_optuna_worker.py \
      --study pr_rrt_abmil_optuna --storage sqlite:///.../x.db --n-trials 15
"""
...
TUNE_DIR = PROJECT / "results" / "c16_pr_rrt_optuna"
FEATURE_BASE = "/home/Public/lillan/features_result/C16_features"

# ── 固定基线（网格已搜/已定，optuna 不碰）──
BASE_CONFIG = {
    "data": {
        "dataset_type": "c16",
        "modalities": ["PR"],
...
def objective(trial):
    ...
    try:
        ...
        # 只保留日志，清掉 ckpt/img 省盘（best config 由 study 记录，最终 5-seed 会重训）
...
def main():
    ...
    print(f"[GPU {gpu}] worker start study={args.study}", flush=True)

    study = optuna.load_study(study_name=args.study, storage=args.storage)
    study.optimize(objective, n_trials=args.n_trials, show_progress_bar=False)

    print(f"[GPU {gpu}] worker done", flush=True)

#!/usr/bin/env python3
"""HE Residual Cross **v3** —— Train / Val-as-Test 协议 driver（3 Model seeds）。

只训练 v3 的 3 个 model seed（42/123/456）。HE-only / v1 / v2 的结果**复用**上一轮：
...
v3 与 v2 唯一差异（模块层面）：
    PR value centering 由「per-slide 减自身均值」改为「减 train-set EMA running
    prototype μ_PR」（β=0.99，train-only 更新，eval 冻结，buffer 不进 optimizer）。
其余（cosine cross-attention τ=0.2、bias-free QKV、两个独立 RRT、独立 HE/PR
routing、Q=HE、K/V=PR、HE identity 残差、residual_scale=0.1、ABMIL、CE-only、
Train/Val-as-Test 协议、LR=1e-4、随机初始化端到端）完全一致。

用法:
  python scripts/run_stage2_v3.py --gpus 0 1 4
"""
...
# Stage1 各自 best —— 与 HE-only / v1 / v2 完全一致，绝对不改
STAGE1_ENCODER_CFG = {
...
# Stage2 固定 r4 no-EPEG；v3 = v2 三改动 + prototype_momentum=0.99（唯一核心修改）
STAGE2_CFG = {
...
CONDITION = "he_residual_cross_v3"
LABEL = "HE Residual Cross v3"
...
def generate_summary():
    ...
    summary = {
        "task": "stage2_he_residual_cross_v3",
        "evaluation_protocol": "Train / Val-as-Test",
        "protocol_note": ("official test set 在训练中被用于 checkpoint selection / early stopping，"
                          "属既定 val-as-test 开发协议，非严格意义的独立 untouched test"),
        "condition": meta,
        "paired_delta_auc": {
            "v3_minus_he": paired_delta(meta["per_seed_auc"], he_only_auc),
            "v3_minus_v1": paired_delta(meta["per_seed_auc"], v1_auc),
            "v3_minus_v2": paired_delta(meta["per_seed_auc"], v2_auc),
        },
        "reused_baselines": {
    ...
    L = []
    L.append("# HE Residual Cross v3 —— Train / Val-as-Test 协议\n")
    ...
    L.append("v3 = v2（cosine τ=0.2 + bias-free QKV）+ dataset-prototype PR value centering"
             "（减 train-set EMA prototype μ_PR 而非 per-slide 均值）。HE-only/v1/v2 复用，不重训。\n")

    L.append("## AUC（逐 seed；HE-only / v1 / v2 复用）\n")
    ...
    L.append("|---|---|---|---|---|")
    for name, key in [("v3 − HE", "v3_minus_he"), ("v3 − v1", "v3_minus_v1"),
                      ("v3 − v2", "v3_minus_v2")]:
    ...
    print(f"[summary] v3 mean AUC = {_fmt(auc_m)}", flush=True)
    print(f"[summary] v3 − HE = {_fmt(summary['paired_delta_auc']['v3_minus_he']['mean_delta'])} | "
          f"v3 − v1 = {_fmt(summary['paired_delta_auc']['v3_minus_v1']['mean_delta'])} | "
          f"v3 − v2 = {_fmt(summary['paired_delta_auc']['v3_minus_v2']['mean_delta'])}", flush=True)
    return summary

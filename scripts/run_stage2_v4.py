#!/usr/bin/env python3
"""HE Residual Cross **v4** —— Train / Val-as-Test 协议 driver（3 Model seeds）。

只训练 v4 的 3 个 model seed（42/123/456）。HE-only / v1 / v2 / v3 的结果**复用**：
  - HE-only 0.8344 / 0.8615 / 0.8143
  - v1      0.8406 / 0.8908 / 0.8551
  - v2      0.8418 / 0.8755 / 0.8139
  - v3      0.8753 / 0.8755 / 0.8724
本轮不重训任何 baseline。

v4 与 v3 唯一差异（模块层面）：
    cross-attention 的 attention score 由
        A = softmax( Q̂ K̂ᵀ / τ ),   Q̂=Q/‖Q‖, K̂=K/‖K‖
    改为
        A = softmax( Q̂_c K̂_cᵀ / τ ),  Q̂_c=Q_c/‖Q_c‖, K̂_c=K_c/‖K_c‖
        Q_c = Q − Q̄, K_c = K − K̄   （Q̄/K̄ = per-slide per-head 有效 routing token 均值）
    即去掉 Q/K 的 slide 内 common direction（只作用于 attention score，V 不变）。
其余（cosine τ=0.2、bias-free QKV、两个独立 RRT、独立 HE/PR routing、Q=HE、
K/V=PR、dataset-prototype PR value centering V'=V−μ_PR、HE identity 残差、
residual_scale=0.1、ABMIL、CE-only、Train/Val-as-Test 协议、LR=1e-4、随机初始化
端到端）与 v3 完全一致。v4 不引入任何新超参（Q/K centering 恒开启）。

用法:
  python scripts/run_stage2_v4.py --gpus 0 1 4
"""
...
MAX_PATCHES = 2500
RESULTS_ROOT = PROJECT / "results" / "stage2_he_residual_cross_v4"
SEED_RUNNER = str(PROJECT / "scripts" / "_run_protocol_seed.py")

# Stage1 各自 best —— 与 HE-only / v1 / v2 / v3 完全一致，绝对不改
STAGE1_ENCODER_CFG = {
...
# Stage2 固定 r4 no-EPEG；v4 = v3 全部配置 + Q/K common-direction removal（恒开启，无新 knob）
STAGE2_CFG = {
...
CONDITION = "he_residual_cross_v4"
LABEL = "HE Residual Cross v4"
...
def generate_summary():
    ...
    # reuse baselines (do NOT retrain)
    prev_summary = json.loads(
        (PROJECT / "results" / "stage2_he_residual_cross_val_as_test" / "summary.json")
        .read_text())
    v2_summary = json.loads(
        (PROJECT / "results" / "stage2_he_residual_cross_v2" / "summary.json")
        .read_text())
    v3_summary = json.loads(
        (PROJECT / "results" / "stage2_he_residual_cross_v3" / "summary.json")
        .read_text())
    ...
    v3_auc = v3_summary["condition"]["per_seed_auc"]
    ...
    summary = {
        "task": "stage2_he_residual_cross_v4",
        "evaluation_protocol": "Train / Val-as-Test",
        "protocol_note": ("official test set 在训练中被用于 checkpoint selection / early stopping，"
                          "属既定 val-as-test 开发协议，非严格意义的独立 untouched test"),
        "condition": meta,
        "paired_delta_auc": {
            "v4_minus_he": paired_delta(meta["per_seed_auc"], he_only_auc),
            "v4_minus_v1": paired_delta(meta["per_seed_auc"], v1_auc),
            "v4_minus_v2": paired_delta(meta["per_seed_auc"], v2_auc),
            "v4_minus_v3": paired_delta(meta["per_seed_auc"], v3_auc),
        },
        "reused_baselines": {
            "he_only": {"per_seed_auc": he_only_auc},
            "he_residual_cross_v1": {"per_seed_auc": v1_auc},
            "he_residual_cross_v2": {"per_seed_auc": v2_auc},
            "he_residual_cross_v3": {"per_seed_auc": v3_auc},
            "source": "results/stage2_he_residual_cross_val_as_test/summary.json + "
                      "results/stage2_he_residual_cross_v2/summary.json + "
                      "results/stage2_he_residual_cross_v3/summary.json",
        },
        "protocol": {
        ...
        "v4_change": "Q/K per-slide per-head common-direction removal on the attention "
                     "score only (V centering unchanged from v3); always-on, no new hyperparam",
    ...
    L = []
    L.append("# HE Residual Cross v4 —— Train / Val-as-Test 协议\n")
    ...
    L.append("v4 = v3（cosine τ=0.2 + bias-free QKV + dataset-prototype PR value centering）"
             "+ Q/K slide-internal common-direction removal（只作用于 attention score）。"
             "HE-only/v1/v2/v3 复用，不重训。\n")

    L.append("## AUC（逐 seed；HE-only / v1 / v2 / v3 复用）\n")
    ...
    L.append(f"| v1 (reuse) | {_fmt(v1_auc['42'])} | {_fmt(v1_auc['123'])} "
             f"| {_fmt(v1_auc['456'])} | 0.8622 ± 0.0211 |")
    L.append(f"| v2 (reuse) | {_fmt(v2_auc['42'])} | {_fmt(v2_auc['123'])} "
             f"| {_fmt(v2_auc['456'])} | 0.8438 ± 0.0252 |")
    L.append(f"| v3 (reuse) | {_fmt(v3_auc['42'])} | {_fmt(v3_auc['123'])} "
             f"| {_fmt(v3_auc['456'])} | 0.8744 ± 0.0014 |")
    ...
    L.append("|---|---|---|---|---|")
    for name, key in [("v4 − HE", "v4_minus_he"), ("v4 − v1", "v4_minus_v1"),
                      ("v4 − v2", "v4_minus_v2"), ("v4 − v3", "v4_minus_v3")]:
    ...
    print(f"[summary] v4 mean AUC = {_fmt(auc_m)}", flush=True)
    for k in ["v4_minus_he", "v4_minus_v1", "v4_minus_v2", "v4_minus_v3"]:
        print(f"[summary] {k} = {_fmt(summary['paired_delta_auc'][k]['mean_delta'])}", flush=True)
    return summary

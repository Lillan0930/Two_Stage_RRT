#!/usr/bin/env python3
"""
PR best config 5-seed 复评（test-as-val，GPU 7）。

加载 tune/best_results/pr_rrt_abmil/best_params.json 的最优参数，跑 5 个 seed，
汇报 test-as-val AUC 的 mean±std，与 Base_mil 的 0.8001±0.0468（同为 test-as-val 泄漏协议）对比。
...
def run_seed(seed: int):
    ...
    logger.info(f"PR best config 5-seed re-eval | seed={seed}")
    ...
    print(f"[seed {seed}] val_auc={result['val_auc']:.4f} "
...
def main():
    print("PR best config 5-seed re-eval (test-as-val, GPU 7)", flush=True)
    print(f"Seeds: {SEEDS}", flush=True)
    print(f"Best params: {json.loads(BEST_PATH.read_text())['best_params']}",
          flush=True)
    ...
    print("\n" + "=" * 64, flush=True)
    print("SUMMARY (test-as-val, 270 train / 129 val-test)", flush=True)
    for r in results:
